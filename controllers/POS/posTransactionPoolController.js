...
module.exports =
{
    ...
    async SendEmailPOS(req, res) {
        try {

            let pos_data = await posTransactionPools.aggregate(
                [
                    {
                        $lookup: {
                            from: "poolwallets", // collection to join
            ...
            {
                "emailTemplateName" : "posreceipt.ejs",
                "to"                : req.body.email,
                "subject"           : "POS Transcation Receipt",
                "templateData":
                {
                    "transid"           : pos_data[0].id,
                    "coin"              : pos_data[0].networkDetails[0].coin,
                    "network"           : pos_data[0].networkDetails[0].network,
                    "storename"         : pos_data[0].storeDetails[0].storename,
                    "amount"            : pos_data[0].amount,
                    "address"           : pos_data[0].poolWallet[0].address,
                }
            }
            ...
            res.json({ status: 200, message: "Message", data: email_response })
